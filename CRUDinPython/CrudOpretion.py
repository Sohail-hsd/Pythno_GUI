...
def Add():
    ...
    mysqldb = mysql.connector.connect(host="localhost", user="root", password="", database="company")
    mycursor = mysqldb.cursor()

    try:
        sql = "INSERT INTO  registration (id,empname,mobile,salary,adress) VALUES (%s, %s, %s, %s,%s)"
        val = (id, empname,mobile, salay,adress)
...
def update():
    ...
    try:
        sql = "Update  registration set empname= %s,mobile= %s,salary= %s, adress= %s where id= %s"
...
Label(root,text="adress").place(x=400,y=130)
...
cols = ('id', 'empname', 'mobile', 'salary','adress')
